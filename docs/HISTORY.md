# История разработки Podshar

Архив: все промпты и подробные записи об изменениях, версии 0.1 - 0.13.

Этот файл НЕ загружается автоматически — в отличие от CLAUDE.md, который
читается целиком в начале каждой сессии. Здесь он лежит, чтобы его можно было
открыть намеренно, когда нужно вспомнить, почему что-то сделано именно так.

То же самое, короче, есть в сообщениях git-коммитов: `git log`.
...
Промпт 12 (версия 0.12 — система входа):

"Делаем систему аккаунтов. Пароль + инвайт, база — Neon."


ИЗМЕНЕНИЯ, ВНЕСЁННЫЕ ПО ПРОМПТУ 12
==================================

База (Neon)
- Проект small-unit-17442214 / production, eu-central-1. `neon link` кладёт
  DATABASE_URL и DATABASE_URL_UNPOOLED в .env.local (в .gitignore вместе с .neon).
- schema.prisma получил directUrl. Приложение ходит через пулер, но DDL через
  пулер идти не должен: PgBouncer в transaction mode не даёт настоящей сессии,
  а prisma migrate опирается на advisory-локи и prepared statements.
- Миграция 20260905194813_init — 56 таблиц. Сид записал три инвайта.

Почему не next-auth
- Его Credentials-провайдер поддерживает ТОЛЬКО стратегию jwt. Модель Session,
  уже лежавшая в схеме, осталась бы мёртвой, а сессии — неотзываемыми. Для
  allowlist на троих это ровно наоборот тому, что нужно. Пакет удалён.
- Своя реализация: 32 случайных байта в куке, SHA-256 в БД. Доступ на чтение к
  базе не даёт рабочую сессию. Выход = DELETE строки.

Файлы
  lib/db.ts             Prisma-синглтон через globalThis (иначе dev-HMR плодит
                        пулы соединений до исчерпания лимита Postgres)
  lib/auth/cookie.ts    ТОЛЬКО имя куки, без импортов. middleware работает в
                        Edge Runtime, где Prisma не грузится вообще; импорт
                        session.ts оттуда сломал бы весь middleware.
  lib/auth/password.ts  argon2id, параметры OWASP (19 МиБ, 2 прохода)
  lib/auth/session.ts   create / read (обёрнут в React cache) / destroy
  lib/auth/actions.ts   login, join, logout — server actions
  components/auth/*     AuthForm, LoginForm, JoinForm, SignOutButton

Группы маршрутов
- [locale]/layout.tsx теперь только документ и провайдер переводов.
- [locale]/(app)/ — шелл (шторка, ассистент, канва) плюс НАСТОЯЩАЯ проверка
  доступа: спрашивает БД, живая ли сессия.
- [locale]/(auth)/ — login и join на голой канве. Уже вошедшего отправляет
  домой: предлагать войти второй раз незачем.

Middleware — не защита, а срез пути
- Он в Edge Runtime, Prisma там нет, поэтому проверяет ТОЛЬКО наличие куки.
  Подделанная кука проходит middleware и разворачивается в (app)/layout.tsx.
  Смысл middleware — не рендерить весь шелл, чтобы потом выкинуть.
- Порядок важен: сначала next-intl (без локали в пути нечего сравнивать с
  PUBLIC, и его редирект должен победить), потом проверка куки.

Детали, которые легко пропустить
- login проверяет пароль ДАЖЕ когда пользователя нет — против настоящего
  argon2-хеша DUMMY_HASH. Иначе несуществующий хендл отвергается заметно
  быстрее неверного пароля, а это способ узнать, какие из трёх существуют.
  Хеш должен быть валидным: выдуманная строка роняет verify на парсинге и
  возвращает мгновенно, то есть возвращает ровно ту разницу во времени.
- join: погашение инвайта и создание пользователя в одной транзакции, причём
  claimedAt: null стоит в ФИЛЬТРЕ updateMany — это же и защита от гонки: при
  двух одновременных отправках строку обновит только одна.
- Первый вошедший получает роль OWNER.
- Rate limit: 8 неудач с адреса за 10 минут, считается по audit_logs. Адрес
  хранится хешем — для «кто-то долбится» нужно только равенство.
- Выход — форма (POST), не onClick: GET-выход срабатывает от чего угодно,
  что рендерит URL, от префетчера до превью ссылки в чате.
- Тексты ошибок намеренно расплывчатые: «неверный хендл или пароль» и «нет
  такого хендла» — одна и та же строка.

Попутно исправлено
- public/icons/* не существовали, манифест на них ссылался: 404 и предупреждение
  в консоли на каждой загрузке. Иконки 192/512/maskable нарисованы по мотивам
  icon.svg (стрелка «Под» над красным шаром «Шар»).

Проверено в браузере, полным циклом
- Регистрация по ВРЕМЕННОМУ инвайту (три настоящих токена не тронуты):
  форма -> редирект на главную -> «Добрый вечер, Tymofii…», профиль из БД.
- Вход по хендлу и паролю -> /ru, шторка на месте. Выход -> /ru/login.
- Неверный пароль -> «неверный хендл или пароль». Погашенный токен ->
  «приглашение неизвестно, просрочено или уже использовано».
- Тестовый пользователь и временный инвайт удалены: в базе 0 пользователей и
  три неиспользованных инвайта (one, two, three).
- tsc --noEmit чисто, npm run build проходит, /ru отдаёт 307 на /ru/login.
- Консоль на всех четырёх локалях и на мобильных 390px — чисто.

ОСТАЛОСЬ ПО АККАУНТАМ
- Профиль: смена имени, языка, часового пояса; аватар (пока выбор из готовых,
  загрузка файлов тянет за собой весь медиа-пайплайн).
- Смена пароля, «запомнить меня».
- Капча из галереи (модель CaptchaChallenge готова, нужна галерея).
- Живые статы: getQuickStats уже читает GameStatSnapshot через GameAccount, но
  писать снимки пока некому.


Промпт 13 (версия 0.13 — свой хостинг):

"Хост был у друга, он его удалил. У него на хосте не было регистрации, а при
попытке зарегаться вылетала Application error. Подними хост на моём аккаунте,
чтобы сайт работал без моего ноутбука."


ИЗМЕНЕНИЯ, ВНЕСЁННЫЕ ПО ПРОМПТУ 13
==================================

Дыра в безопасности, найденная на живом хосте
- До этого правило было одно: «нет DATABASE_URL — не проверяем ничего», чтобы
  сайт запускался до появления базы. На ноутбуке это удобство. На хосте друга
  это означало, что приватный сайт на троих отдавался всему интернету: любой
  открывал podshar-website.vercel.app и ходил по нему как Guest, потому что
  одна незаданная переменная молча выключала все проверки.
- Появился src/lib/auth/config.ts. Правило стало асимметричным:
    development, нет базы — открыто (защищать нечего, снаружи не достать)
    production,  нет базы — ЗАКРЫТО, всех на /login
    есть база                — работает настоящая проверка
  Сломанный сайт — испорченный вечер, открытый приватный сайт — утечка.
- Страница /login остаётся доступной и без базы: именно туда неверно
  настроенный деплой отправляет людей, и она должна хотя бы отрисоваться.
- Та самая Application error со скриншота была следствием того же: без
  DATABASE_URL любое обращение к Prisma на сервере падает.

Хостинг переехал на аккаунт trqwaa
- vercel link → проект trqwaa/podshar-website. Домен podshar-website.vercel.app
  освободился после удаления проекта друга и достался новому.
- В переменные окружения (production и preview) добавлены DATABASE_URL и
  DATABASE_URL_UNPOOLED из .env.local. Их отсутствие и было причиной всех бед.
- npm run deploy = vercel deploy --prod.

Автодеплой пока не подключён
- `vercel git connect` не проходит: репозиторий принадлежит NellleT, а Vercel
  аккаунт — trqwaa, прав нет. Варианты: NellleT добавляет trqwaa в
  коллабораторы, либо передаёт репозиторий, либо деплоим вручную.

Проверено на живом хосте
- /ru отдаёт 307 на /ru/login: гость внутрь не попадает.
- /uk/login (страница из скриншота с ошибкой) отдаёт 200.
- Отправка формы входа с несуществующим хендлом вернула «неверный хендл или
  пароль», и в audit_logs Neon появилась строка login_failed с User-Agent
  браузера — то есть прод реально достучался до базы и проверил argon2.
  Тестовые строки login_failed удалены, чтобы не заряжать rate limit.
- В базе: 1 пользователь (@one), 2 свободных инвайта.

Попутно
- Vercel CLI дописал в .gitignore шаблон .env*, который заодно накрыл
  .env.example. Добавлено исключение !.env.example — в шаблоне секретов нет и
