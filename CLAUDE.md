...
КАК ОТКАТИТЬСЯ
==============
Проект до этого момента не был под гитом. Репозиторий заведён специально ради
этого промпта.

  main               — состояние v0.6, коммит 5b2c835, тег v0.6
  experiment/less-serious — всё, что описано ниже

  Откат целиком:        git switch main
  Посмотреть разницу:   git diff main experiment/less-serious
  Забрать один файл:    git checkout main -- src/components/PHButton.tsx
  Принять эксперимент:  git switch main && git merge experiment/less-serious

Ничего из v0.6 не потеряно: main стоит на том же коммите, что и до эксперимента.
