...
export default async function HomePage({
  ...
  return (
    <div className="grid grid-cols-1 content-start gap-3 p-3 sm:gap-4 sm:p-4 md:grid-cols-6">
      ...
      <section className="block-card animate-rise-in flex items-center justify-center bg-canvas-sunk px-4 py-10 [animation-delay:60ms] md:col-span-4 md:row-span-2">
        <PHButton />
      </section>
      ...
      <section className="block-card animate-rise-in flex flex-col gap-4 p-6 [animation-delay:120ms] md:col-span-2">
        ...
        <div>
          <p className="text-2xl font-light leading-tight text-ink">
      ...
      <section className="block-card animate-rise-in flex flex-col gap-4 p-6 [animation-delay:180ms] md:col-span-2">
        ...
        <div className="flex items-center gap-3">
          <span
            aria-hidden="true"
            className="grid h-10 w-10 shrink-0 place-items-center border border-ink bg-sand text-sm font-medium text-ink"
          >
            {member.displayName.slice(0, 1).toUpperCase()}
          </span>
          <div className="min-w-0">
            <p className="truncate text-xl font-light leading-tight text-ink">
              {member.displayName}
            </p>
            <p className="truncate text-sm text-ink-muted">@{member.handle}</p>
          </div>
        </div>
      </section>
    </div>
